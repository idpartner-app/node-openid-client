...
<form method="post" action="${this.issuer.authorization_endpoint}">
  ...
  async backchannelAuthentication(params = {}, { clientAssertionPayload } = {}) {
    assertIssuerConfiguration(this.issuer, 'backchannel_authentication_endpoint');
    ...
    const responseBody = processResponse(response, { statusCode: 200 });
    console.log(`responseBody`, responseBody);

    if (!('expires_in' in responseBody)) {
      throw new RPError({
        message: 'expected expires_in in Backchannel Authentication Successful Response',
        response,
      });
    }
    if (typeof responseBody.expires_in !== 'number') {
      throw new RPError({
        message: 'invalid expires_in value in Backchannel Authentication Successful Response',
        response,
      });
    }
    if (!('auth_req_id' in responseBody)) {
      throw new RPError({
        message: 'expected auth_req_id in Backchannel Authentication Successful Response',
        response,
      });
    }
    if (typeof responseBody.auth_req_id !== 'string') {
      throw new RPError({
        message: 'invalid auth_req_id value in Backchnanel Authentication Successful Response',
        response,
      });
    }
  ...
  async pollBackchannelAthentication(
    body,
    checks = {},
    { exchangeBody, clientAssertionPayload, DPoP } = {},
  ) {
